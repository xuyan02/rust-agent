...
pub trait Tool: Send + Sync {
    ...
    async fn invoke(&self, workspace: &Path, function_name: &str, args: &Value) -> Result<Value>;
}
...
impl Tool for DebugTool {
    ...
    async fn invoke(&self, _workspace: &Path, function_name: &str, args: &Value) -> Result<Value> {
        match function_name {
            "debug.echo" => {
                let text = args
                    .get("text")
    ...
    async fn invoke(&self, workspace: &Path, function_name: &str, args: &Value) -> Result<Value> {
        match function_name {
            "file.read" => {
                ...
                Ok(
                    serde_json::json!({"ok": true, "data": {"path": path, "content": s, "truncated": false}}),
                )
            }
            "file.write" => {
                let path = args
                ...
                Ok(
                    serde_json::json!({"ok": true, "data": {"path": path, "bytes": content.len(), "overwrote": overwrite}}),
                )
            }
            _ => bail!("unknown function: {function_name}"),
        }
