...
async fn main() -> anyhow::Result<()> {
    ...
    // 4. Create contexts without dir_node - they inherit from Session
    {
        println!("Context 1 (inherits Session's dir_node):");
    ...
    {
        println!("\nContext 2 (also inherits Session's dir_node):");
        let history: Box<dyn History> = Box::new(PersistentHistory::new());
    ...
    // 5. Create context with its own dir_node (overrides Session)
